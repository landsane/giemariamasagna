...
export default function MembreFormModal({ initial, onClose, onSaved }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 sm:p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md overflow-hidden max-h-[95dvh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          {/* Code membre (attribué automatiquement, lecture seule) */}
          {editing && (
            <div className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 flex items-center justify-between">
              <p className="text-xs text-gray-500">Code membre</p>
              <p className="text-sm font-mono font-bold text-gray-700">{initial!.id_membre}</p>
            </div>
          )}
